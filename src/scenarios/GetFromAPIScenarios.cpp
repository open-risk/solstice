...
void Scenarios::GetFromAPI(std::string &URL, Simulation &W, Model &M, Poco::LogStream &logstream) {
    ...
    try {
        if (LOG_LEVEL >= 2) {
            logstream.information() << ">> 4 | Fetching scenario data via API " << std::endl;
        }
        ...
        if (LOG_LEVEL >= 2) {
            logstream.information() << ">> 4 | Scenario Data Server Response " << std::endl;
            if (LOG_LEVEL >= 3) {
