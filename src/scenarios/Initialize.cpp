...
void Scenarios::Initialize(Simulation &W, Model &M, Poco::LogStream &logstream) {

    if (LOG_LEVEL >= 2) {
        logstream.information() << ">> 1 | Scenario Data from API URL's / Local Dir Paths" << std::endl;
    }
    ...
    if (operating_mode == 0 || operating_mode == 3) {
        std::string filename;
        if (W.getPath_mode() == 1) {
            filename = W.DataDir() + "/Scenarios/" + W.ScenarioDataURL();
        }
        ...
        logstream.information() << ">> 3 | Read Scenario Data from API URL: " << URL << std::endl;
    }
}