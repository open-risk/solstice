...
void ModelComponent::Initialize(const std::string& fragment, Simulation &W, Model &M, Poco::LogStream &logstream) {

    if (LOG_LEVEL >= 2) {
        logstream.information() << ">> 1 | Fetching Model Components from API URL's / Local Dir Paths" << std::endl;
    }
    ...
    if (operating_mode == 0 || operating_mode == 3) {
        ...
        if (W.getPath_mode() == 1) {
            filename = W.DataDir() + "/Model_Data/" + fragment + ".json";
